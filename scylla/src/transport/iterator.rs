...
use futures::Stream;
use scylla_cql::frame::frame_errors::{ResultMetadataLazyDeserializationError, RowsParseError};
    ...
    fn poll_next_page<'r>(
        ...
        let raw_rows_with_deserialized_metadata =
            received_page.rows.deserialize_metadata().map_err(|err| {
                NextRowError::NextPageError(NextPageError::ResultMetadataDeserializationError(err))
            })?;
...
pub enum NextPageError {
    ...
    ResultMetadataDeserializationError(#[from] ResultMetadataLazyDeserializationError),
