    ...
    DeserApi: DeserializationApiKind,
{
    ...
    async fn try_getting_tracing_info(
        ...
        let maybe_tracing_info: Option<TracingInfo> = traces_session_res
            .into_rows_result()
            .map_err(|err| {
                ProtocolError::Tracing(
                    TracingProtocolError::TracesSessionResultMetadataDeserializationError(err),
                )
            })?
        ...
        let tracing_event_rows_result = traces_events_res
            ...
            .map_err(|err| {
                ProtocolError::Tracing(
                    TracingProtocolError::TracesEventsResultMetadataDeserializationError(err),
                )
            })?
            .ok_or(ProtocolError::Tracing(
